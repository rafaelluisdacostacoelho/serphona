...
- **Grounded + Governed**: Retrieval (RAG) for knowledge, MCP for actions with schemas/authz/audit; fail-closed on low confidence.
...
### With tools-gateway / MCP:
- Resolve and invoke governed tools via MCP (e.g., billing.get_invoice, crm.lookup_customer) with tenant-scoped tokens.
- Enforce tool allow/deny by agent role/environment; capture tool call logs (status, latency, input/output hash) for audit.
...
### Retrieval (RAG) / Tool (MCP) Failures:
- Retrieval low score or timeout: do not inject context; ask clarifying question or transfer (fail-closed for voice).
- MCP/tool error/denied: surface safe message, retry when idempotent, otherwise transfer.
- Always log retrieval chunk IDs and tool call status for audit.
