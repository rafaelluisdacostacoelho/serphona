...
### MCP e Catálogo Governado
- Tools Gateway serve como fonte para catálogo MCP (resources expostos via `resource = <namespace.action>`), mantendo schemas de entrada/saída, timeouts e política de idempotência.
- Policy engine por tenant/agent/environment: allow/deny por tool, escopos do token (platform-auth), rate limits e quotas por tenant.
- Contratos padronizados de erro (código + mensagem), logging com hashes de input/output, latência e status para auditoria e billing.
- Integração com RAG: ferramentas `rag_query` podem ser expostas aqui para harmonizar faturamento/observabilidade.
