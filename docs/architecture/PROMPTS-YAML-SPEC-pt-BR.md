...
```yaml
# prompts.yaml schema
version: "1.1"            # 1.1 adds RAG + MCP fields (backward compatible with 1.0)
tenant_id: "uuid"
...
global:
  ...
  rag_defaults:               # Optional global defaults for RAG
    enabled: true
    top_k: 4
    rerank: true
    threshold: 0.35           # similarity threshold (0-1)
    namespaces: ["default"]  # logical KB namespaces (per-tenant filtered)
  ...
  - id: "unique-agent-id"
    ...
    tools:
      ...
      - name: "billing_get_invoice"
        type: "mcp"            # MCP resource (preferred for governed tooling)
        enabled: true
        config:
...
### RAG + MCP Extensions (v1.1)
- `global.rag_defaults`: shared retrieval defaults (top_k, rerank, threshold, namespaces, citations_required).
- `agents[].knowledge.rag`: per-agent retrieval config; filters must enforce `tenant_id` and channel/language; `fallback: fail_closed` recommended for voice when context is low quality.
- `agents[].tools[].type: mcp`: MCP-governed tools; use `config.resource`, `timeout`, optional `idempotency_key`; schemas remain required for inputs/outputs.
- Require citations in system prompts when `citations_required: true`; include snippet IDs in responses for audit.
- Keep conversation memory separate from KB: RAG pulls corporate docs; session state stays in orchestrator/DB.
