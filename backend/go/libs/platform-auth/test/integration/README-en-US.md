...
## How to run
- Windows: `backend\go\libs\platform-auth\test\integration\run-integration-tests.bat` (add `--with-compose` to reuse the root test stack).
- Linux/macOS: `backend/go/libs/platform-auth/test/integration/run-integration-tests.sh` (use `--with-compose` when you want `docker-compose.tests.yml`).
