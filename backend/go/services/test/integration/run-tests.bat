...
rem Runs Go service test suites (integration, e2e) with optional docker-compose stack.
...
set SERVICES=tenant-manager auth-gateway billing-service analytics-query-service agent-orchestrator tools-gateway voice-gateway
for %%S in (%SERVICES%) do (
  if /I "%SUITE%"=="integration" call :run_suite integration %%S
  if /I "%SUITE%"=="e2e" call :run_suite e2e %%S
  if /I "%SUITE%"=="all" (
    call :run_suite integration %%S
    call :run_suite e2e %%S
  )
)
...
set "SUITE_NAME=%~1"
set "SVC_NAME=%~2"
set "SVC_DIR=%ROOT_DIR%\backend\go\services\%SVC_NAME%"
set "TEST_DIR=!SVC_DIR!\test\%SUITE_NAME%"

if not exist "!TEST_DIR!" (
  echo [skip] %SVC_NAME% has no test\%SUITE_NAME%; skipping
  goto :eof
)

dir /b "!TEST_DIR!\*_test.go" >nul 2>&1
if errorlevel 1 (
  echo [skip] %SVC_NAME% test\%SUITE_NAME% has no *_test.go; skipping
  goto :eof
)

echo [info] running %SUITE_NAME% tests for %SVC_NAME%
pushd "!SVC_DIR!" >nul
go test -tags=%SUITE_NAME% ./test/%SUITE_NAME% %GO_ARGS%
set "TEST_RESULT=!ERRORLEVEL!"
popd >nul
if not "%TEST_RESULT%"=="0" (
  echo [error] %SVC_NAME% %SUITE_NAME% tests failed with code %TEST_RESULT%
  if "%STACK_STARTED%"=="1" if not "%KEEP_COMPOSE%"=="1" %COMPOSE_CMD% -f "%COMPOSE_FILE%" down
  popd >nul
  endlocal & exit /b %TEST_RESULT%
)

echo [info] %SVC_NAME% %SUITE_NAME% tests passed
goto :eof
