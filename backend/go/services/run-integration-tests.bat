...
rem Runs integration tests for Go services using the integration build tag.
rem Usage: run-integration-tests.bat [--with-compose] [--keep-compose] [go test args]
rem Notes:
rem - With --with-compose, starts the root docker-compose.tests.yml (Kafka stack only).
rem - Service-specific deps (e.g., DATABASE_URL) must be provided by env or your own compose stack.
...
for %%S in (%SERVICES%) do (
  set SVC_DIR=%ROOT_DIR%\backend\go\services\%%S
  set TEST_DIR=!SVC_DIR!\test\integration

  if not exist "!TEST_DIR!" (
    echo [skip] %%S has no test\integration; skipping
    goto next_svc
  )

  dir /b "!TEST_DIR!\*_test.go" >nul 2>&1
  if errorlevel 1 (
    echo [skip] %%S integration dir has no *_test.go; skipping
    goto next_svc
  )

  echo [info] running integration tests for %%S
  pushd "!SVC_DIR!" >nul
